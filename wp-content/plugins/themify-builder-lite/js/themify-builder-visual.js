                ...
                for (var k in this.bp_reverse) {
                    ...
                    if(media_v){
                            ...
                            if (index === false || !rules[index]) {
                                ...
                                rules[index].style[prop] = media_v;
                            }
                            ...
                            if (this.prevData[api.activeModel.cid][bp][index] === undefined) {
                            ...
                            this.tempData[bp][index][prop] = media_v;
                    }
                }
            }
        ...
        ThemifyLiveStyling.prototype.addOrRemoveFrame = function ( $option ) {
			...
			if ( settings.repeat ) {
				if ( side === 'left' || side === 'right' ) {
					self.setLiveStyle( 'background-size', '100% ' + ( ( 100 / settings.repeat ) + 0.1 ) + '%', selector );
				} else {
					self.setLiveStyle( 'background-size', ( ( 100 / settings.repeat ) + 0.1 ) + '% 100%', selector );
				}
			} else {
        ...
        ThemifyLiveStyling.prototype.bindChangesEvent = function () {
            ...
            self.$context.on('change', self.style_tab + ' select,' + self.style_tab + ' textarea,' + self.style_tab + ' input.tb_uploader_input,' + self.style_tab + ' input[type="radio"]', function (e) {
                ...
                if (is_select) {
                    ...
                    }else if ( cl.contains('font-weight-select') && ! cl.contains('disable-font-weight') ) {

						var $fontFamily = $(this).closest('.tb_field').prev('.tb_field'),
							request = $fontFamily.find('select').val() + ':' + $val ;
